//
//  MySynth1ExtensionDSPKernel.hpp
//  MySynth1Extension
//
...
/*
 MySynth1ExtensionDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class MySynth1ExtensionDSPKernel {
public:
    ...
    // Add a case for each parameter in MySynth1ExtensionParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case MySynth1ExtensionParameterAddress::gain:
                mGain = value;
                break;
        }
    }
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        auto visitor = [] (void* context, MIDITimeStamp timeStamp, MIDIUniversalMessage message) {
            auto thisObject = static_cast<MySynth1ExtensionDSPKernel *>(context);
